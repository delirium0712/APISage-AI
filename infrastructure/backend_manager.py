...
class BackendManager:
        ...
        discovery_configs = [
            {
                "name": "ollama",
                "backend_type": "llm",
                "config": {"api_base": "http://localhost:11434", "provider": "ollama", "model": "llama3:8b"},
    ...
    async def _connect_llm_provider(self, config: Dict[str, Any]):
        ...
        try:
            from .llm_providers.base_provider import LLMProvider, LLMConfig
            
            # Convert provider string to enum
            try:
                provider_enum = LLMProvider(config.get("provider", "ollama"))
            except ValueError:
                self.logger.error("invalid_provider_type", provider=config.get("provider", "ollama"))
                return None
